...
static struct list_head retrans_timer_list; // lab16 added
...
// lab16 added:5 functions
// scan the retrans_timer_list, find the tcp sock which retrans 3 times, release it
	...
	list_for_each_entry_safe(t, q, &retrans_timer_list, list) {
		...
		if(t->timeout <=0){
			if((t->retrans_times >= MAX_RETRANS) && (tsk->state != TCP_CLOSED)){
				...
				if ((t->retrans_times > 0) && (tsk->state != TCP_CLOSED))
					printf("** Retrans times: %d **\n", t->retrans_times);
				t->retrans_times += 1;
...
// set the retrans timer of a tcp sock, by adding the timer into retrans_timer_list
...
// update the retrans timer of a tcp sock
...
// unset(delete) the retrans timer of a tcp_sock
...
// scan the retrans_timer_list periodically by calling tcp_scan_timer_list
