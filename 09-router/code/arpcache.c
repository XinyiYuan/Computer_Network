...
void arpcache_append_packet(iface_info_t *iface, u32 ip4, char *packet, int len)
{
	...
	cached_packet = (struct cached_pkt*)malloc(sizeof(struct cached_pkt));
...
void *arpcache_sweep(void *arg) 
{
	while (1) {
		sleep(1);
		// fprintf(stderr, "lab09 added: sweep arpcache periodically: remove old entries, resend arp requests .\n");
